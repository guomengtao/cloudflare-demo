...
date_of_birth : 提取 Date of Birth  后的日期，格式化为 YYYY-MM-DD 。
missing_since : 提取 Missing Since  后的日期，格式化为 YYYY-MM-DD 。
age_at_missing : 提取 Age  后的纯数字。
...
location_details : 提取 Missing From  后的完整原始地点描述。
sex : 提取 Sex  后的文本。
race : 提取 Race  后的文本。
height : 提取 Height and Weight  中的身高部分。
weight : 提取 Height and Weight  中的体重部分。
eye_color : 从 Distinguishing Characteristics  文本中分析并提取眼色。
hair_color : 从 Distinguishing Characteristics  文本中分析并提取发色。
distinguishing_marks : 提取 Distinguishing Characteristics  后的完整描述（含scar痕、纹身等）。
vehicle_info : 从 Details of Disappearance  中提取提到的车型/车牌，无则填 NULL 。
classification : 提取 Classification  后的文本。
investigating_agency : 提取 id="agencies"  下的机构名称。
source_info : 提取 Source Information  底部所有链接的文本，逗号分隔。
images_json : 提取 id="photos"  内所有真实出现的图片的 src 属性 (图片URL)，只接受以.jpg结尾的图片URL，必须是HTML中真实存在的图片，不能返回任何假的或占位符图片URL！将所有符合条件的图片URL整理为JSON字符串数组格式，例如：["https://example.com/image1.jpg", "https://example.com/image2.jpg"]。⚠️ 绝对不允许返回包含对象的数组，只允许返回包含URL字符串的数组！⚠️ 绝对不允许返回任何非.jpg结尾的图片URL！⚠️ 绝对不允许返回占位符图片URL！
disappearance_details : 提取 Details of Disappearance  之后所有 <p>  标签的原始文本。
total_updates_count : 提取底部 .updated  中 Updated X times  的数字。
last_case_update_raw : 提取页面底部关于更新时间的原始描述字符串。
last_verified_date : 提取 Last updated  或 Last verified  后的日期，转为 YYYY-MM-DD 。
💡 处理建议
空值处理 ：在 Prompt 中要求 AI 若 HTML 中未找到某字段，一律返回 null 。
    ...
    try {
        ...
        try {
            ...
            return {
                ...
                case_id: caseId,
                data: {
                    full_name: aiData.full_name || null,
                    date_of_birth: aiData.date_of_birth || null,
                    missing_since: aiData.missing_since || null,
                    age_at_missing: aiData.age_at_missing || null,
                    missing_city: aiData.missing_city || null,
                    missing_county: aiData.missing_county,
                    missing_state: aiData.missing_state || null,
                    location_details: aiData.location_details || null,
                    sex: aiData.sex || null,
                    race: aiData.race || null,
                    height: aiData.height || null,
                    weight: aiData.weight || null,
                    eye_color: aiData.eye_color || null,
                    hair_color: aiData.hair_color || null,
                    distinguishing_marks: aiData.distinguishing_marks || null,
                    vehicle_info: aiData.vehicle_info || null,
                    classification: aiData.classification || null,
                    investigating_agency: aiData.investigating_agency || null,
                    source_info: aiData.source_info || null,
                    main_photo_url: main_photo_url,
                    images_json: processedImages.length > 0 ? processedImages : [],
                    disappearance_details: aiData.disappearance_details || null,
                    total_updates_count: aiData.total_updates_count || null,
                    disappearance_details_word_count: calculateWordCount(aiData.disappearance_details),
                    last_case_update_raw: aiData.last_case_update_raw || null,
                    last_verified_date: aiData.last_verified_date || null
                }
            };
        } catch (parseError) {
