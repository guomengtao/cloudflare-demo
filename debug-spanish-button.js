...
async function debugSpanishButton() {
    console.log('🔍 调试西班牙语版本Submit按钮...');
    ...
    try {
        ...
        console.log('✅ 西班牙语页面加载成功');
        
        // 截图初始状态
        await page.screenshot({ path: 'debug-spanish-before.png', fullPage: true });
        ...
        const spanishSelectors = [
            'input[type="submit"]',
            'button[type="submit"]',
            'input[value*="Enviar"]', // 西班牙语"Submit"
            'input[value*="Buscar"]', // 西班牙语"Search"
            'button[value*="Enviar"]',
            'button[value*="Buscar"]',
            '.btn-primary',
            '.search-button'
        ...
        if (!clicked) {
            ...
            for (const button of allButtons) {
                ...
                const spanishKeywords = ['enviar', 'buscar', 'enviar', 'búsqueda'];
        ...
        if (clicked) {
            // 等待结果加载
            console.log('⏳ 等待西班牙语结果加载...');
            await page.waitForTimeout(5000);
            
            // 截图点击后的状态
            await page.screenshot({ path: 'debug-spanish-after.png', fullPage: true });
            ...
            const hasResults = await page.evaluate(() => {
                ...
                return text.includes('NCMC') || 
                       text.includes('AMBER') ||
                       text.includes('caso') || // 西班牙语"case"
                       text.includes('desaparecido') || // 西班牙语"missing"
                       document.querySelector('.search-results, .results, tbody, table') !== null;
            });
            
            console.log(hasResults ? '✅ 西班牙语结果已加载' : '❌ 未检测到西班牙语结果');
            ...
            // 查找案件相关元素
            const caseElements = await page.$$('tr, .item, .card, .poster-card');
...
debugSpanishButton().catch(console.error);const { chromium } = require('playwright');
